...
import com.example.admin.dailyquote2.services.QuoteDowloadByRetrofit;
...
public class QuoteApplication extends Application {
    ...
    public void onCreate() {
        ...
        if (NetworkManager.getInstance().isConnectedToInterner()) {
            Log.d(TAG, "Connected");
            Intent quoteIntent = new Intent(this, QuoteDowloadByRetrofit.class);
            startService(quoteIntent);

            Intent unplashIntent = new Intent(this, UnplashDowloadService.class);
            startService(unplashIntent);
        } else {
